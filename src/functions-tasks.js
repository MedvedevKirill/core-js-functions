...
function getPolynom(...params) {
  return (x) =>
    params.reduce(
      (sum, param, index) => sum + param * x ** (params.length - 1 - index),
      0
    );
}
...
function memoize(func) {
  ...
  return (...params) => {
    ...
    if (cache.has(paramsString)) {
      return cache.get(paramsString);
    }
    const newResult = func(...params);
    cache.set(paramsString, newResult);
    return newResult;
  };
}
...
function retry(func, attempts) {
  return (...params) => {
    function makeAttempt(_func, _attempts) {
      try {
        return func(...params);
      } catch (err) {
        if (_attempts > 0) {
          return makeAttempt(_func, _attempts - 1);
        }
        throw err;
      }
    }
    return makeAttempt(func, attempts);
  };
}
...
function logger(func, logFunc) {
  return (...params) => {
    const functionString = `${func.name}(${params.map((item) => JSON.stringify(item)).join(',')})`;
